...
func interceptLoadConfig() (conf *cfg.Config, err error) {
	...
	// create a default carnot config file if it does not exist
	carnotConfigFilePath := filepath.Join(rootDir, "config/carnot.toml")
	if _, err := os.Stat(carnotConfigFilePath); os.IsNotExist(err) {
		carnotConf, _ := config.ParseConfig()
		config.WriteConfigFile(carnotConfigFilePath, carnotConf)
	}
